...
def insert_random_people():
    ...
    favorite_animals = ['Dog', 'Cat', 'Camel', 'Chicken', 'Alpaca', 'Meercat']
    for user in users:
        animal_ind = randint(0, len(favorite_animals)-1)
        score = randint(0,100)
        money = randint(0,10000)
        
        person = {
            "name": user,
            "favorite_animal": favorite_animals[animal_ind],
            "score": score,
            "money": money
        }
...
def insert_users_and_summaries():
    users = ['Jacob', 'Chris', 'Varun', 'Osama', 'Stefan']
    random_summaries = [
        "The dog chased after the cat who chased the mouse",
        "Some men just want to watch the world burn",
        "Do or do not, there is no try",
        "Sauce or no sauce, there is always sauce",
        "Blah Blah Blah",
        "Bock bock bacoooookk",
    ]


    for user in users:
        num_sums = randint(0, len(random_summaries)-1)
        person = {
            "userid": user,
            "summaries": []
        }
        for i in range(num_sums):
            person["summaries"].append(random_summaries[i])
